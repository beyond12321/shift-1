					...
					<form class="form-horizontal" action="<%=EMP_EDIT_URL%>" id="myform" method="post" enctype="multipart/form-data">
						<fieldset>
							...
							<div class="form-group" id="eidDiv">
								...
								<div class="col-md-5">
									<!-- 定义表单输入组件 -->
									<input type="text" id="eid" name="eid" class="form-control" value="${emp.eid}" readonly>
								</div>
								<!-- 定义表单错误提示显示元素 -->
							...
							<div class="form-group" id="passwordDiv">
								...
								<div class="col-md-5">
									...
									<input type="text" id="password" name="password" class="form-control"
										placeholder="请输入雇员登录密码" value="hello">
								</div>
								<!-- 定义表单错误提示显示元素 -->
							...
							<div class="form-group" id="jidDiv">
								...
								<div class="col-md-5">
									<select id="jid" name="jid" class="form-control">
										<c:choose>
											<c:when test="${empLevel.level < level}">
												<!-- 要修该的雇员的等级比登录的雇员要高, 此时只列出，不能选择 -->
												<option value="${empLevel.lid}" selected>${empLevel.title}</option>
											</c:when>
											<c:otherwise>
												<c:forEach items="${allLevels}" var="lev">
													<c:if test="${lev.level >= level}">
